...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'SEO AI-Visibility');
}
...
// Wave-2 CostModel (§4). seo-ai-visibility (LLM responses) ≈ $0.0034 flat per
// prompt → ~60 credits. MEASURE one prompt (read tasks[0].cost) and set base =
// ceil(cost×15000) before PLATFORM_DATAFORSEO_AUTH goes live. Only charged when
// our platform key backs the call (BYO is free).
export default { tools, callTool, cost: { base: 43 }, provider: 'dataforseo' } satisfies McpToolExport;
